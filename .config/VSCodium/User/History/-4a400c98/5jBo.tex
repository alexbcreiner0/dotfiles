...
\begin{frame}{Dynamic Programming Summary}
    \begin{itemize}
        \item I'm a little bit unsold on giving this topic it's own name. What we are talking about is just simply recursion. 
        \item With dynamic programming/recursion, we are identifying a way to break a problem down in terms of a set of subproblems. I suppose that dynamic programming could be considered more general insofar as recursion specifically assumes that these subproblems are sub-instances of \emph{the same} problem, but no examples of the paradigm I've seen do otherwise, and so the terminology seems pointless.
        \item Often times, the answer is specifically the biggest or smallest result from a collection of \emph{possible} subproblems, as was the case for shortest paths in dags. 
    \end{itemize}
\end{frame}
