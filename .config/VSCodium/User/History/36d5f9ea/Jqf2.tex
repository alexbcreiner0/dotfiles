...
\begin{frame}{Knapsack - An interesting case}
    \begin{itemize}
        \item Denote the original problem we had $K(n,W)$ If we take the item, then we are left with the subproblem $K(n-1,W-w)$. If we don't, then we are left with $K(n-1,W)$. Thus we have the formula:
