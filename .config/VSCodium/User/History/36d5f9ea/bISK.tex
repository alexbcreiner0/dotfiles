...
\begin{frame}{Normalizing SAT}
    \begin{itemize}
        ...
        \item 3SAT is really the big important problem that we want to reduce other problems from and to. The extra assumed structure is usually helpful to have. Again if you would like to see proofs for these things, take my class in the spring!
    \end{itemize}
\end{frame}
...
\begin{frame}{Proving Hardness and Coping}
    \begin{itemize}
        ...
        \item Suppose we find a way to 3SAT \emph{to} our problem (not the other way around!). Then we've shown that our problem is \emph{at least as hard} as 3SAT, in that a solution to our problem could be used to solve 3SAT with no significant time loss.
        \item At that point, we know that our problem is $\mathbf{NP}$-hard, since 3SAT itself is $\mathbf{NP}$-complete. This is an indication that you should give up on trying to solve it in the normal ways we've been approaching problems in this class. What are the non-normal ways?
    \end{itemize}
\end{frame}
