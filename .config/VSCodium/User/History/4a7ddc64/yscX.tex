...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \end{itemize} 
    \end{itemize}
    \item[(3)] Consider the 8x8 chess board pictured below. The red circle represents a knight chess piece, and the 8 X marks represent the different legal moves that a chess piece can make. Label 
    \begin{center}
...
\begin{tikzpicture}[scale=0.8]
    \foreach \x in {0,...,7} {
        \foreach \y in {0,...,7} {
            ...
            \draw[\colour] (\x,\y) rectangle ++(1,1);
        }
    }
    
    \foreach \x in {0,...,8} {
        \node[left] at (-0.5,\x-0.5) {\x};
        \node[above] at (\x-0.5,8.5) {\x};
    }
\end{tikzpicture}
